...
def generate_expected(input_file, output_file):
    """Generate expected sorted output using Python's built-in sort."""
    with open(input_file) as fin:
        numbers = [int(line.strip()) for line in fin if line.strip()]
    numbers.sort()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for dist in DISTRIBUTIONS:
            ...
            try:
                ...
                print(f"  FAIL  {algo} + {dist}  (comparison error: {e})")
                failed += 1
